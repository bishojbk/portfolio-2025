      ...
      <motion.div onViewportEnter={() => setSelectedPage("project")}>
        <div className="grid grid-cols-2 gap-6 sm:gap-8 md:gap-10 w-full mx-0 sm:mx-5">
          {projectData.map((value, index) => (
            <div
              className="relative h-[300px] lg:h-[600px] max-w-full project-item group"
              key={index}
            >
              <motion.img
                src={value.imgUrl}
                alt={value.title}
                className="object-cover max-w-full project-image h-[280px] lg:!h-[580px] rounded-[16px] shadow-md"
                initial="hidden"
                whileInView={"visible"}
                viewport={{ once: true, amount: 0.5 }}
